...
/**
 * @description
 	重写Textview ，解决android旧版本的bug: 设置 android:ellipsize="end" 和 android:maxLines="4" >2 的时候。
 	只会显示两行内容末尾带省略号。
 * @author chendt
 * @date 2013-11-11
 * @Version 1.0
  ...
  private void resetText() {
    ...
    if (layout.getLineCount() > linesCount) {
      ...
      while (createWorkingLayout(workingText + ELLIPSIS).getLineCount() > linesCount) {
    	...
    	// 由于我们大多数情况下workingText为中文，所以按照上面的逻辑找空格是不合适的
    	// 这里改成直接替换最后的字符 workingText = workingText.substring(0, workingText.length() - 1 - 1); 
