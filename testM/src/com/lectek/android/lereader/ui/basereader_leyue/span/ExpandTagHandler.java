...
public class ExpandTagHandler implements TagHandler{
	...
	public boolean handleTag(TagInfo tagInfo,Editable editable,boolean isStart) {
		if(isStart){
			if(tagInfo.getTag().equals("note")){
				String note = tagInfo.getAttributes().getValue("", "value");
				if(!TextUtils.isEmpty(note)){
					tagInfo.addStyle(new NoteSpan(note));
					editable.append(Constant.REPLACEMENT_SPAN_CHAR);
				}
			}else if("true".equalsIgnoreCase(tagInfo.getAttributes().getValue("data-audioRef"))){
			...
			}else if(tagInfo.getTag().equals("audios")){
				String title = tagInfo.getAttributes().getValue("", "title");
				String src = tagInfo.getAttributes().getValue("", "src");
				if(!TextUtils.isEmpty(src)){
					String img = tagInfo.getAttributes().getValue("", "img");
					AsyncDrawableSpan span = null;
					if(!TextUtils.isEmpty(img)){
						Integer size = null;
						float width = 0;
						float height = 0;
						size = tagInfo.handleSize(tagInfo.getWidthValue());
						if(size != null){
							width = size;
						}
						size = tagInfo.handleSize(tagInfo.getHeightValue());
						if(size != null){
							height = size;
						}
						span = new AsyncDrawableSpan(img,title, false,width, height, mDataProvider);
					}
					tagInfo.addStyle(new VoicesSpan(src,span));
					editable.append(Constant.REPLACEMENT_SPAN_CHAR);
				}
			}
		}else{
