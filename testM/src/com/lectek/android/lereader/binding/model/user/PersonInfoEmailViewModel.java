...
public class PersonInfoEmailViewModel extends BaseLoadDataViewModel {
	...
	public final OnClickCommand bSaveClick = new OnClickCommand() {
		...
		public void onClick(View v) {
			...
			if(CommonUtil.checkEmail(email)){
				...
				saveUserInfo(mUserId, bNickName.get(), mAccount, mPassword, mPassword, new ISaveUserInfoHandler() {
					...
					public void saveUserInfoSuccess() {
						...
						if(mUserAction != null){
							...
							userInfo.setEmail(bNickName.get());
				...
				//非正确邮箱地址格式的号码
				ToastUtil.showLongToast(getContext(), R.string.error_email_tip);
			}
			
		}
	};

	public PersonInfoEmailViewModel(Context context, INetLoadView loadView) {
...
//			changeLoginState(false);
	...
	private void refreshView(){
		if(mDataSource != null){
			if(!TextUtils.isEmpty(mDataSource.getEmail())){
				bNickName.set(mDataSource.getEmail());
	...
	private void saveUserInfo(final String userId, final String nickName, final String userName, final String password, final String newPassword, final ISaveUserInfoHandler saveUserInfoHandler){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					result = ApiProcess4Leyue.getInstance(getContext()).updateUserInfo(userId, null, userName, password, newPassword, null, nickName, null, null, null, null);
