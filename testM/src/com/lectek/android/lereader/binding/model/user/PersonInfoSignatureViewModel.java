...
/**
 * 账户管理---个性签名信息ViewModel
 * 
 * @author yangwq
 * @date 2014年7月14日
 * @email 57890940@qq.com
 */
public class PersonInfoSignatureViewModel extends BaseLoadDataViewModel {
...
//			mUserAction.showKeyBoardAction();
				...
				saveUserInfo(mUserId, bNickName.get(), mAccount, mPassword, mPassword, new ISaveUserInfoHandler() {
					...
					public void saveUserInfoSuccess() {
						...
						if(mUserAction != null){
							...
							userInfo.setSignature(bNickName.get());
	...
	public PersonInfoSignatureViewModel(Context context, INetLoadView loadView) {
		super(context, loadView);
		mUserInfoModel = new UserInfoModelLeyue();
		mUserInfoModel.addCallBack(this);
		bAttributeTip.set(getString(R.string.dialog_person_signature));
	}
...
//			changeLoginState(false);
	...
	private void refreshView(){
		if(mDataSource != null){
			if(!TextUtils.isEmpty(mDataSource.getSignature())){
				bNickName.set(mDataSource.getSignature());
	...
	private void saveUserInfo(final String userId, final String nickName, final String userName, final String password, final String newPassword, final ISaveUserInfoHandler saveUserInfoHandler){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					result = ApiProcess4Leyue.getInstance(getContext()).updateUserInfo(userId, null, userName, password, newPassword, null, null, null, null, null, nickName);
