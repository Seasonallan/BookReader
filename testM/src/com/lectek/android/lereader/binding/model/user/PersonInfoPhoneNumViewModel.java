...
import com.lectek.android.lereader.utils.PhoneNumUtil;
import com.lectek.android.lereader.utils.ToastUtil;

public class PersonInfoPhoneNumViewModel extends BaseLoadDataViewModel {
	...
	public final OnClickCommand bSaveClick = new OnClickCommand() {
		...
		public void onClick(View v) {
			...
			if(PhoneNumUtil.isPhoneNumber(phoneNum)){
				...
				saveUserInfo(mUserId, bNickName.get(), mAccount, mPassword, mPassword, new ISaveUserInfoHandler() {
					...
					public void saveUserInfoSuccess() {
						...
						if(mUserAction != null){
							...
							userInfo.setMobile(bNickName.get());
				...
				//非正确手机号格式的号码
				ToastUtil.showToast(getContext(), R.string.error_phone_num_tip);
			}
			
		}
	};

	public PersonInfoPhoneNumViewModel(Context context, INetLoadView loadView) {
		super(context, loadView);
		mUserInfoModel = new UserInfoModelLeyue();
		mUserInfoModel.addCallBack(this);
		bAttributeTip.set(getString(R.string.dialog_person_mobile));
	}
...
//			changeLoginState(false);
	...
	private void refreshView(){
		if(mDataSource != null){
			if(!TextUtils.isEmpty(mDataSource.getMobile())){
				bNickName.set(mDataSource.getMobile());
	...
	private void saveUserInfo(final String userId, final String nickName, final String userName, final String password, final String newPassword, final ISaveUserInfoHandler saveUserInfoHandler){
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					result = ApiProcess4Leyue.getInstance(getContext()).updateUserInfo(userId, null, userName, password, newPassword, nickName, null, null, null, null, null);
